document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderHijriCalendar(date) {
    ...
    hijriMonthTitle.innerText = `${hijriMonthNames[hijriMonth - 1]} ${hijriYear} H`;
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      const div = document.createElement("div");
      div.className =
        "py-1 rounded relative cursor-pointer hover:bg-primary";
      ...
      const todayParts = getHijriParts(today);
      const isToday =
        todayParts.day === i &&
